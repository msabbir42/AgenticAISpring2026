...
# Normal input is sent to exactly one model:
#   - if the input begins with "Hey Qwen", it goes to Qwen
#   - otherwise, it goes to Llama
...
class AgentState(TypedDict):
    ...
    Graph flow:
        START -> get_user_input -> [conditional]
                                  ...
                                  |-> call_llama ----------> print_selected_response -> get_user_input
                                  |-> call_qwen  ----------> print_selected_response -> get_user_input
    """
    user_input: str
    should_exit: bool
    trace_enabled: bool
...
def create_graph(llama_llm, qwen_llm):
    ...
    2. call_llama: Sends input to the Llama model
    3. call_qwen: Sends input to the Qwen model
    4. print_selected_response: Prints the response from the selected model
    ...
    def print_selected_response(state: AgentState) -> dict:
        """
        Print the response from whichever model branch just ran.
        """
        if state.get("trace_enabled", False):
            print("[TRACE] Entering node: print_selected_response")

        if state.get("qwen_response", ""):
            print("\n" + "-" * 50)
            print("Qwen response")
            print("-" * 50)
    ...
    def route_after_input(state: AgentState) -> str:
        ...
        - "call_qwen": if the input begins with "Hey Qwen"
        ...
        stripped_input = state.get("user_input", "").strip()
        ...
        if stripped_input.lower().startswith("hey qwen"):
            if state.get("trace_enabled", False):
                print("[TRACE] Router selected: call_qwen")
            return "call_qwen"
    ...
    graph_builder.add_node("print_selected_response", print_selected_response)
    ...
    graph_builder.add_edge("call_llama", "print_selected_response")
    graph_builder.add_edge("call_qwen", "print_selected_response")
    graph_builder.add_edge("print_selected_response", "get_user_input")
...
def main():
    ...
    """
    print("=" * 50)
    print("LangGraph Simple Agent with Routed Llama and Qwen")
    print("=" * 50)
    print()
